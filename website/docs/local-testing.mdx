...
6. In **Authentication → Required actions**, open **⚙️** on **Set up Email Authenticator** and enable the masked-email setting (enrollment sending settings can also be configured here; if none are saved, enrollment falls back to the first Email OTP execution config)
7. Start enrollment: expected masked destination on the verification screen, independently of the login setting
