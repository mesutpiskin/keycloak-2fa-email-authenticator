...
If you want to show the destination email address on the OTP screens, prefer the built-in **Show masked email on OTP form** settings instead of relying on `${user.email!}` in the FreeMarker template:

- on the Email OTP execution, for the **login** OTP form
- on the **Set up Email Authenticator** required action, for the **enrollment** verification form

When enabled, a server-generated `maskedEmail` value is passed to the form template. This is more reliable across Keycloak versions because the login template context does not always expose a populated `user` object during the OTP step.
