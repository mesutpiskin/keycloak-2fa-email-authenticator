...
| **Show masked email on OTP form** | Show a masked version of the destination email on the OTP **login** screen | `false` | `true` |
...
It applies to the **login** OTP screen only. Enrollment has a separate toggle on the required action (see below).
...
Enrollment (`Set up Email Authenticator`) is configured under **Authentication → Required actions**, not on the Email OTP execution.

| Setting | Description | Default |
| --- | --- | --- |
| **Email provider** | Provider used to send the enrollment verification code | `KEYCLOAK` |
| **SendGrid / AWS SES / Mailgun** | Provider credentials and sender settings | — |
| **Enable fallback to Keycloak SMTP** | Fall back to realm SMTP when the primary provider fails | `true` |
| **Code length** | Number of digits in the enrollment code | `6` |
| **Time-to-live** | Seconds before the enrollment code expires | `300` |
| **Simulation mode** | Log codes to server logs instead of sending emails | `false` |
| **Resend cooldown** | Seconds before the user can request a new enrollment code | `30` |
| **Max code attempts** | Wrong attempts before the enrollment code is invalidated | `5` |
| **Show masked email on OTP form** | Show a masked destination on the enrollment verification screen after the code is sent | `false` |

The masked-email toggle is independent of the same-named setting on each Email OTP execution. Enable both if the masked address should appear at login **and** during enrollment.

### Fallback to Email OTP execution config

If no enrollment sending setting is saved on the required action, enrollment falls back to the first `email-authenticator` or `conditional-email-authenticator` execution found in the realm (legacy behaviour). As soon as any sending key is set on the required action, that config is used for the whole enrollment flow (provider, TTL, code length, cooldown, max attempts, simulation mode).

Configure enrollment explicitly on the required action when the realm has more than one Email OTP execution, or when login and enrollment should use different providers or TTLs.
